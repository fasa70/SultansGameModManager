...
`app/src/main/python`, plus a Catalog-backed adapter which copies selected Mod
inputs into a temporary workspace. It never loads or ships game-original JSON
and does not invoke the upstream desktop base/delta workflow.

The separate `tools/sultan-core-wheel` staging directory contains the minimum
upstream JSON/`json_ops` source set for a Chaquopy Android wheel. Its source
locks are:
