...
class DirectWorkshopDownloader(
        ...
        for (attempt in 1..MAX_DIRECT_DOWNLOAD_ATTEMPTS) {
            ...
            try {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    if (totalBytes != null && totalBytes > MAXIMUM_DIRECT_FILE_SIZE_BYTES) {
                        throw WorkshopDownloadException("Direct download exceeds the size limit")
                    }
                    ...
                    response.body?.byteStream()?.use { input ->
                        FileOutputStream(partialFile, append).buffered().use { output ->
                            ...
                            while (true) {
                                ...
                                written += read
                                if (written > MAXIMUM_DIRECT_FILE_SIZE_BYTES) {
                                    throw WorkshopDownloadException("Direct download exceeds the size limit")
                                }
                                emit(
                                    DownloadEvent.Progress(
    ...
    companion object {
        ...
        private const val MAXIMUM_DIRECT_FILE_SIZE_BYTES = 512L * 1024L * 1024L
    }
}
